...
class AlbertDistill(AlbertPreTrainedModel):
    """Pre-trained Albert model that uses our loss functions"""
    def __init__(self, config, num_labels_, loss_fn: ClfDistillLossFunction):
        super(AlbertDistill, self).__init__(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        _, pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, return_dict=False, \
...
if __name__=='__main__':
    from clf_distill_loss_functions import Plain
    model = AlbertDistill.from_pretrained('albert-base-v2',num_labels_=3, loss_fn=Plain)