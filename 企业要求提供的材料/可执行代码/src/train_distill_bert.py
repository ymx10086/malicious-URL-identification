...
from bert_distill import BertDistill, AlbertDistill
...
def main():
    ...
    parser.add_argument("--bert_model", default="bert-base-uncased", type=str,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                             ...
                             "bert-base-multilingual-cased, bert-base-chinese, albert-base-v2.")
    ...
    if "-uncased" in args.bert_model:
        ...
        do_lower_case = False
    elif args.bert_model == "albert-base-v2":
        do_lower_case = True
    else:
        raise NotImplementedError(args.bert_model)

    if args.bert_model == "albert-base-v2":
        tokenizer = AlbertTokenizer.from_pretrained(args.bert_model, do_lower_case=do_lower_case,mirror='tuna')
    else:
    ...
    if args.bert_model == "albert-base-v2":
        model = AlbertDistill.from_pretrained(
            args.bert_model, cache_dir=cache_dir, num_labels_=3, loss_fn=loss_fn, \
                attention_probs_dropout_prob=0.1, hidden_dropout_prob =0.1)
    else:
        model = BertDistill.from_pretrained(
            args.bert_model, cache_dir=cache_dir, num_labels=3, loss_fn=loss_fn)

    unfrozen_name = ['embeddings', 'dense', 'layer.10', 'layer.11', 'pooler','classifier']
    logging.info("ALL Parameters: ")
 
# frozen some parameter  
    for name, param in model.named_parameters():
        param.requires_grad = False
        for elem in unfrozen_name:
            if elem in name or not args.frozen:
    ...
    if args.do_train:
        ...
        if 'albert' not in args.bert_model:
            config = BertConfig(output_config_file)
            model = BertDistill(config, num_labels=3, loss_fn=loss_fn)
            model.load_state_dict(torch.load(output_model_file))
        else:
            config = AlbertConfig.from_json_file(output_config_file)
            model = AlbertDistill(config, num_labels_=3, loss_fn=loss_fn)
            model.load_state_dict(torch.load(output_model_file))
...
"""
python train_distill_bert.py \
  --bert_model albert-base-v2 \
  --output_dir ../experiments_self_debias_mnli_seed111/albert_none \
  --do_train --do_eval --mode reweight_by_teacher_annealed \
  --custom_teacher ../teacher_preds/mnli_trained_on_sample2K_seed111.json --seed 111 --which_bias hans --fp16
python train_distill_bert.py \
  --bert_model albert-base-v2 \
  --output_dir ../experiments_self_debias_mnli_seed111/albert_none \
  --do_eval --mode reweight_by_teacher_annealed \
  --custom_teacher ../teacher_preds/mnli_trained_on_sample2K_seed111.json --seed 111 --which_bias hans --fp16
"""